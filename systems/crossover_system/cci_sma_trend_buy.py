"""A simple moving average cross or cci strategy. This strategy buys when the cci goes from below to above 100, or when the fast sma crosses above the slow sma. It sells when the cci goes from above to below -100, or when the slow sma crosses above the fast sma. Only buys with a longer term trend. If already in a position, it either holds if given the same signal, or reverses the position if given the opposite. There are no stops. Cope."""
...
class CciSmaSystemTrendBuy(FuturesSystem):
    ...
    def __repr__(self) -> str:
        return f"Buy CCI Trend ({self.fast_avg}) ({self.slow_avg}) ({self.trend_length}) ({self.cci_length})"
    ...
    def check_buy(self):
        buy = ((self._check_cci_buy() or self._check_sma_buy()) and self._check_trend_confirm_buy()) or self._check_trend_change_buy()
        close = self._check_cci_buy() or self._check_sma_buy()
        if buy and self.position == "NONE":
            ...
            self.last_difference = self.order_price
            return True 
        elif close and self.position == "SHORT":
            self.profit += ((self.order_price - self.linedict[self.time][1]) * self.position_size * self.contract_multiplier) 
            self.position = "NONE"
    ...
    def check_sell(self):
        sell = self._check_cci_sell() or self._check_sma_sell()
    ...
    def write_file(self):
        c = '_'
        name = self.symbol + c + str(self.fast_avg) + c + str(self.slow_avg) + c + str(self.trend_length) + c + str(self.cci_length) + "_trend_buy_report.txt"
        outfile = open(name, 'w') 
