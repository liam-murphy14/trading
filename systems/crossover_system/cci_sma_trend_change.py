"""A simple moving average cross or cci strategy. This strategy buys when the cci goes from below to above 100, or when the fast sma crosses above the slow sma. It sells when the cci goes from above to below -100, or when the slow sma crosses above the fast sma. Only trades with a longer term trend. If already in a position, it either holds if given the same signal, or reverses the position if given the opposite. There are no stops. Cope."""
...
class CciSmaSystemTrendChange(FuturesSystem):
    ...
    def __repr__(self) -> str:
        return f"Change CCI Trend ({self.fast_avg}) ({self.slow_avg}) ({self.trend_length}) ({self.cci_length})"
    ...
    def write_file(self):
        c = '_'
        name = self.symbol + c + str(self.fast_avg) + c + str(self.slow_avg) + c + str(self.trend_length) + c + str(self.cci_length) + "_trend_change_report.txt"
        outfile = open(name, 'w') 
